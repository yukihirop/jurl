...
pub fn print_response(stdout: &[u8], raw: bool) -> std::io::Result<()> {
    ...
    if let Some((code, time)) = s.status.as_ref().filter(|(c, _)| c != "000") {
        ...
        let line = format!("HTTP {code} · {ms}ms");
        if tty && !raw {
            writeln!(o, "{line}")?;
        } else {
            writeln!(e, "{line}")?;
    ...
    if tty && !raw {
        if let Ok(v) = serde_json::from_slice::<serde_json::Value>(s.body) {
            writeln!(o, "{}", serde_json::to_string_pretty(&v).unwrap_or_default())?;
            return Ok(());
        }
    }
    o.write_all(s.body)?;
...
pub fn explain(tokens: &[Token], jev: Option<&JevInfo>) {
    ...
    let _ = writeln!(e, "{:<w$}  {:<16} {:<5} {:<4}  note", "word", "role", "conf", "by", w = w);
    for t in tokens {
        let role = t.role.map(|r| r.key().to_string()).unwrap_or_else(|| "?".into());
        ...
        let _ = writeln!(e, "{:<w$}  {:<16} {:<5.2} {:<4}  {}", t.text, role, t.confidence, by, note, w = w);
    }
    let _ = match jev {
        Some(j) => {
        ...
        None => writeln!(e, "jev: not called (fast path)"),
    };
}
